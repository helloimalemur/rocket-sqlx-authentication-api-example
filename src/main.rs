...
use crate::manage_sessions::session_funcs::{
    get_username_by_session, verify_session_by_session_id,
};
...
async fn get_user(
    ...
    if key.to_string().eq(secret) {
        result = get_username_by_session(session_id, pool).await;
...
pub async fn main() {
    ...
    custom(&config)
        ...
        .mount(
            "/",
            routes![index, adduser, login, logout, verify_by_session, get_user,],
        )
        .attach(CORS)
        .launch()
        .await
